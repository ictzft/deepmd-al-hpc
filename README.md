...
当前阶段主要在 **2×Tesla V100 GPU** 平台上完成项目原型验证，包括：
...
- top-K 高不确定性构型筛选；
- dataset-level offline active learning 多轮闭环验证；
- Round 0–3 summary 与 learning curve 结果整理。

截至 **2026-05-17**，项目已经从“单模型 baseline + 模拟 committee forces”推进到“真实 committee models + 真实 model deviation + dataset-level offline active learning 多轮闭环 + learning curve 结果分析”。当前已经完成 Round 0、Round 1、Round 2 和 Round 3 的数据扩展、committee model 训练、冻结、测试、候选构型筛选和结果汇总。
...
本项目不直接使用 Megatron 训练 DeePMD，也不直接复用 Megatron 的 tensor parallel / pipeline parallel 算子，而是借鉴其中的系统设计思想，解决机器学习势函数主动学习闭环中的类似问题：
...
这样可以避免前期大量真实 DFT 计算开销，先验证主动学习闭环、数据更新逻辑和高性能训练框架是否可行。
...
- `experiments/` 中的大型训练日志、`.pb` 模型、checkpoint、`.npy` 和 `.npz` 文件不提交到 GitHub；
- GitHub 中只保留代码、配置文件和轻量实验摘要；
- `exp_008_round001_committee_prediction/selected_topk.json` 和 `exp_010_round002_committee_prediction/selected_topk.json` 已作为轻量结果文件纳入版本管理；
- `exp_007`、`exp_009` 和 `exp_011` 中保留轻量 `metrics_summary.md`，用于说明对应轮次的 committee retraining 实验。
...
- [x] 配置 `.gitattributes`，统一脚本换行格式；
- [x] 清理并移除已被 Git 跟踪的 `__pycache__`、`.pyc` 和误生成的根目录空文件；
...
- [x] 将 Round 0–3 配置、轻量结果、summary、figures 和 analysis 脚本 commit 并 push 到 GitHub；
- [x] 将 `exp_008` 和 `exp_010` 的 `selected_topk.json` 纳入版本管理，使 README 中的实验目录与 GitHub 实际文件保持一致。
...
Round 1 top-10 选择结果基于 `round_001_candidate` 的 frame index，轻量记录文件为：
...
Round 2 top-10 选择结果基于 `round_002_candidate` 的 frame index，轻量记录文件为：
...
Round 3 top-10 选择结果基于 `round_003_candidate` 的 frame index，轻量记录文件为：
...
少量已追踪的 JSON 文件用于实验记录和结果复现；后续新增实验仍应优先提交 `metrics_summary.md`、`selected_topk.json`、summary CSV / MD 和轻量 figures，避免提交大型模型和中间数组。
...
如果在宿主机环境中没有 `python` 命令，请使用 `python3`；如果在 DeepMD Docker 容器中执行，则通常可以直接使用 `python`。
...
<!-- README updated after repository metadata cleanup and Round 0-3 V100 active learning summary on 2026-05-17. -->