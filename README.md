...
> 在 rMD17 ethanol 单体系实验中，uncertainty-based selection 相比 random baseline 表现出更稳定的 RMSE 改善趋势：uncertainty Force RMSE 单调下降（0.374→0.354 eV/Å），independent test 确认提升（0.344→0.327 eV/Å），而 random baseline Round 3 的 Force RMSE 为 0.607 ± 0.385 eV/Å，方差较大。toy H2 上四策略对比已完成（差异在 1σ 以内）。多体系验证和 diversity/trust_level baselines 仍待完成。
