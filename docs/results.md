...
本文档汇总 `deepmd-al-hpc` 在 toy H2、rMD17 ethanol 和 rMD17 benzene 上的主动学习实验结果，说明当前能够支持的结论与仍需补充的实验。
...
four-strategy comparison (uncertainty / random / diversity / trust_level)
...
diversity / trust_level baseline (seed0/1/2 Round 001–003)
independent test evaluation
four-strategy comparison
MD stability (NVE 2.5ps)
```
...
```text
数据格式转换 + train/valid/test/candidate 数据划分
  ↓
uncertainty branch Round 000–003 (1000→4000 训练帧, 60000→57000 候选帧)
  ↓
random baseline seed0/1/2 Round 001–003 (done)
  ↓
diversity / trust_level baseline seed0/1/2 Round 001–003 (done)
  ↓
independent test evaluation (done)
  ↓
four-strategy comparison (done)
  ↓
MD stability (NVE 2.5ps, done)
  ↓
GPU utilization profiling + batch-size scaling (done)
```
...
## 12. rMD17 Benzene 真实数据集结果 (2026-06-28)
...
### 12.2 Uncertainty Branch Round 000–003

| Round | Train | Candidate | Selection | Committee models | Status |
|---:|---:|---:|---|---:|---|
...
- 与 rMD17 ethanol 使用相同的 active learning pipeline
- `DP_INFER_BATCH_SIZE=1800` 避免 V100 16GB OOM
- 每轮选择 top-1000 高不确定性构型
...
uncertainty branch Round 000–003 ✓
random baseline seed0/1/2 Round 001–003 ✓
diversity baseline seed0/1/2 Round 001–003 ✓
trust_level baseline seed0/1/2 Round 001–003 ✓
independent test evaluation ✓
four-strategy comparison ✓
MD stability (NVE 2.5ps) ✓
```

### 12.4 四策略 Force RMSE（Round 3, 3-seed mean ± std）
...
- 全部四策略在 benzene 上 Force RMSE 接近（0.182-0.217 eV/Å），uncertainty 略优
- 与 ethanol 不同（active 策略显著优于 random），benzene 上差异较小

### 12.5 MD Stability（NVE 2.5ps, 0.5fs, T_init=100K）

| Model | Drift (eV/atom/ps) | Temp (K) | Stable |
|---|---:|---:|---|
...
- 全部四策略模型在 NVE 下稳定，drift < 0.002 eV/atom/ps（ethanol 约为 0.035 eV/ps）

### 12.6 与 rMD17 ethanol 的对比

| 指标 | rMD17 ethanol | rMD17 benzene |
|---|---|---|
| 原子数 | 9 | 12 |
| 候选池 | 60,000 | 60,000 |
| Uncertainty branch | Round 0–3 (done) | Round 000–003 (done) |
| Random baseline | done | done |
| Diversity baseline | done | done |
| Trust_level baseline | done | done |
| Independent test | done (10000 frames) | done |
| MD stability | done (10K stable) | done (2.5ps stable) |
| Four-strategy comparison | done | done |
...
17. rMD17 benzene 上 uncertainty branch Round 000–003 已完成（4 rounds, 4 models each, top-1000 per round）。
18. rMD17 benzene uncertainty branch、random baseline、diversity baseline、trust_level baseline、independent test、MD stability 全部已完成。
```
...
2. 当前方法在真实 DFT / AIMD 数据集上有效（rmd17 ethanol + benzene 两个体系，四策略 multi-seed multi-round 均已验证）；
...
rMD17 benzene 上四策略（uncertainty / random / diversity / trust_level）multi-seed multi-round 全部已完成，independent test 和 MD stability 已完成；
...
1. toy H2 数据集仅用于流程验证；rMD17 ethanol 和 benzene 两个真实分子体系均已提供四策略 multi-seed multi-round 证据；
...
4. rMD17 benzene 四策略 multi-seed multi-round 全部已完成，independent test 和 MD stability 已完成；仅 H100 和更多分子体系待补充；
5. toy H2 和 rMD17 ethanol 上 diversity 和 trust-level 均已完成 multi-seed multi-round 验证；
6. 当前尚未进行 H100 / 多 GPU scaling 实验；
7. V100 training wall-clock profiling 已全量记录（52 模型）；GPU utilization 曲线未记录；
8. MD 稳定性在 10K NVE 通过验证，100K+ 解离——需提高模型精度后重新评估；
9. 在 rMD17 ethanol 单体系实验中，uncertainty 相比 random 表现出更稳定的改善趋势；random 跨 seed 方差较大，multi-system 验证仍需补充。
...
### 16.2 Four-Strategy Comparison（已完成）
...
### 16.4 rMD17 Ethanol 真实数据集（已完成主要部分）
...
### 16.5 rMD17 Benzene 真实数据集（已启动）

```text
rMD17 benzene data conversion ✓
train / candidate / test split ✓
uncertainty branch Round 000–003 ✓
random baseline (seed0/1/2 Round 001–003) ✓
independent test evaluation ✓
...
当前 benzene 四策略 multi-seed multi-round 全部完成，independent test 和 MD stability 已完成。
...
```text
toy workflow validation + multi-molecule validation (rMD17 ethanol + rMD17 benzene)
```

而不是：

```text
final paper-level validation on real datasets (more systems needed beyond ethanol + benzene)
```
