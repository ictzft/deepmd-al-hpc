...
> ⚠️ **venue（2026-07-19 用户确定）**：投 **IPDPS 2027**（CCF-B，submit ~2026-10 / notify ~2027-01-02 / 会议 2027-06 Seattle；HPC 应用+系统对口）。时间窗口 **3 个月（紧）**，大体系（S12）须尽快补。备选 HPDC（CCF-B，notify 3-31，7 月窗口更从容）。CCGrid 是 CCF-C（曾误判，已弃）。本文档已全文对齐 IPDPS 口径（§5 时间线 / §6 风险 / 附录 A 均按 IPDPS 日期）；§2 Gap 分析保留为 2026-07-19 立项时的历史快照。

> **目标**：IPDPS 2027（CCF-B，HPC 应用+系统对口），预计 2026-10 投稿 / 2027-01 录用（详见上方 venue 横幅）。
...
**主语从「active learning 方法」变成「系统性能」**。方法是背景与实验载体，系统贡献是卖点。这是能否被 IPDPS 接收的关键转换。
...
IPDPS：**Parallel Processing / HPC / Distributed Computing**。本工作落在"异构/分布式系统上的科学计算应用优化"，IPDPS 历史接收大量 AI4Science + 系统性能优化论文，与本题对口。备选 HPDC（CCF-B，7 月窗口更从容）；CCGrid 为 CCF-C，已弃（见上方 venue 横幅）。
...
## 5. 时间线（立项 2026-07 → 投稿 2026-10，IPDPS 2027）

对齐 IPDPS 2027 deadline（abstract/paper ~2026-10，需官网核实）。窗口约 **3 个月（紧）**。

**P1–P4 + 补强 #11–#14 已完成**（2026-07-19，见进度速览）；下表保留立项时的阶段划分并标注当前状态，末尾为剩余冲刺窗口：

| 阶段 | 时间（立项计划） | 里程碑 | 产出 | 状态 |
|---|---|---|---|---|
| **P1 基础设施 + 强 scaling** | 07 下 – 09 上 | `concurrent_runner.py` 上线；1/2/4/8 GPU 强+弱 scaling | Fig.3, Fig.4 | ✅ done |
| **P2 瓶颈 + 优化** | 09 下 – 10 上 | Nsight 根因；≥1 优化实现 + 量化 | Fig.5, Fig.6（C2/C3） | ✅ done |
| **P3 baseline + 大体系** | 10 下 – 11 上 | DP-GEN 对比；1–2 个更大体系 | Tab.2, Fig.7（C4） | 🟡 DP-GEN✅ / 大体系待补 |
| **P4 写作 + 统计** | 11 下 – 12 上 | 论文初稿；多 seed + CI；跨架构表 | 全文 + Tab.1 | 🟡 统计✅ / 正文未开始 |
| **剩余冲刺（IPDPS）** | 08 – 10 | 🔴 S12 大体系 + S15 论文正文 → figure 定稿 | 投稿前 checklist | ⏳ 进行中 |
| **投稿** | ~2026-10 | IPDPS 2027 abstract + paper | 提交 | — |
...
| 风险 | 应对 |
|---|---|
| IPDPS 2027 CFP 日期变动 / deadline 早于预期 | 密切关注官网；**HPDC 2027**（submit ~2027-02，notify ~2027-03-31）作退路 |
...
- 全部文档体系 + 诚实的 claim boundary → 可复现 artifact 优势（IPDPS 加分项）
...
## 附录 A：IPDPS 2027 关键日期（预测，须官网核实）

与上方 venue 横幅一致（基于 IPDPS 历届规律 + 当前 CFP 信息）：

- Abstract / Paper deadline：~2026-10
- Notification：~2027-01-02
- Conference：2027-06，Seattle

退路：HPDC 2027（submit ~2027-02 / notify ~2027-03-31，CCF-B）。

## 附录 B：CCF 等级与 Track 参考

- IPDPS：CCF-B（体系结构/并行与分布/存储系统）← **目标会议**
- HPDC：CCF-B（同上）← 备选
- CCGrid：CCF-C（曾误判为 B，已弃）
- ICDCS：CCF-B（网络/系统）
