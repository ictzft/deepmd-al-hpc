...
> **2026-07-19 重大更新**：项目重新定位为 **IPDPS 2027 系统性能论文**（committee AL for MLP 的 GPU 性能表征与优化）。系统实验见 §1A；下方 §2+ 为历史方法学实验（保留为论文背景/appendix）。

---

## 1A. IPDPS 2027 系统论文进度（2026-07-19，8×5090）
...
| S6 | batch×MPS 组合（SM 4.0%→39.4%，9.9×）| done |
...
| 21 | rMD17 ethanol random baseline | 3 seeds × 3 rounds (36 models), uncertainty shows more stable improvement trend | done |
...
| 28 | rMD17 benzene diversity baseline | 3 seeds × 3 rounds (36 models), F_RMSE=0.1891, MD stable | done |
| 29 | rMD17 benzene trust_level baseline | 3 seeds × 3 rounds (36 models), F_RMSE=0.1966, MD stable | done |
| 30 | rMD17 benzene four-strategy comparison | aligned comparison done, all 4 strategies within 0.02 eV/Å | done |
| 31 | rMD17 benzene MD stability | NVE 2.5ps stable, all strategies drift < 0.002 eV/atom/ps | done |
...
V100 上的所有 Tier 1 和 Tier 2 实验均已完成（2026-06-28）：

1. (done) rMD17 benzene diversity / trust_level baselines (36+36 models)
2. (done) rMD17 benzene four-strategy comparison
3. (done) rMD17 benzene MD stability (NVE 2.5ps, all strategies stable)
4. (done) GPU utilization/memory curves for benzene prediction
5. (done) Prediction batch-size / candidate-size scaling

仍有待完成的仅在 H100 平台（见 section 8）。
...
**Cannot claim (yet):**
- Results generalize broadly (only 2 rMD17 systems tested; benzene four-strategy comparison done but more systems needed)
...
10. (done) rMD17 benzene: diversity / trust_level baselines + MD stability
11. (done) GPU utilization/memory curves for benzene prediction (nvidia-smi dmon)
12. (done) I/O breakdown and prediction batch-size / candidate-size scaling
13. H100 scaling
